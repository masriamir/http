 ...
 * @author Amir
 */
abstract class AbstractHttpCallable implements HttpExecutor, Callable<HttpResponse> {
	private static final Logger LOGGER = LoggerFactory.getLogger(AbstractHttpCallable.class);
	...
	public AbstractHttpCallable(final String url, final List<Header> headers, final List<NameValuePair> parameters, final String method) {
	...
	public HttpResponse call() throws Exception {
		...
		// send the request
		try {
			LOGGER.info("attempting to execute http {} request to {}", method, url);
			...
			try {
				...
				response = new HttpResponse(resp.getAllHeaders(), statusLine, data);
	...
	protected void addPostParameters(final HttpEntityEnclosingRequest request) {
		if (validList(parameters)) {
			LOGGER.debug("setting the following entity parameters: {}", parameters);
	...
	protected void addRequestParameters(final HttpRequestBase request) {
		if (validList(parameters)) {
			try {
				LOGGER.debug("setting the following url parameters: {}", parameters);
				final URIBuilder uriBuilder = new URIBuilder(url).setParameters(parameters);
