 ...
 * default, all fields expected to be translated must be annotated with {@link RequestParameter}.
 *
 * @author Amir
...
public interface RequestObject {
  ...
  /**
   * Returns a Predicate to post-filter the generated parameter map after a {@link #translate()} is
   * executed.
   * <p>
   * Note that by default a Predicate is returned that will remove all parameters with a null or
   * empty value.
   *
   ...
   * <p>
   * This method uses each Field's {@link RequestParameterAdapter} to convert its value to a
   * String.
   *
  ...
  /**
   * Generate a Map of request parameters and their values from all request parameter fields. The
   * Map is then post-filtered based on the Predicate returned by {@link #postFilterParameters()}.
   *
   * @return a map containing all request parameters and their values
   *
   * @throws HttpRequestTranslationException if any errors occur while translating fields
   */
